...
- Implement gravity such that the player falls toward the bottom of the screen
  ...
  zero so that gravity does not continue to accelerate the player downard
...
-----------------------------------------------------------------
                        notes
-----------------------------------------------------------------
- make the goomba walk and finish the complete generalization of the sCollisione method ;) 
- take a relook at the replay lecture and perhaps implement some
- fix the movement thing (could in the following way: when a key is pressed automatically disable all the other keys that could be already be in UP status at the same time) perhaps could use some algorith specified in the std for searching in the status of the key pressed or some like that
