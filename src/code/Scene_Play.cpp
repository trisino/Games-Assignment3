      ...
      switch (level_entities) {
        ...
        default:
          ...
          entity->addComponents<CTransform>(
              gridToMidPixel((float) j * 64, (float) i * 64, entity));
    ...
    if (level_entities < eLevelEntities::LAST && level_entities >= 0) {
      switch (level_entities) {
        ...
        default:
          ...
          // if (entity->id() == TILE)
          //   entity->addComponents<CBoundingBox>(
          //       entity->getComponent<CAnimation>().animation.getSize());// has to change have to adapt a correct bounding
...
void Scene_Play::spawnBullet(const std::shared_ptr<Entity> &entity) {
  ...
  bullet->addComponents<CBoundingBox>(
      bullet->getComponent<CAnimation>().animation.getSize() / 2);
...
void Scene_Play::spawnEnemy() {
  ...
  enemy->addComponents<CTransform>(
  ...
  enemy->addComponents<CBoundingBox>(
  ...
  for (auto &dynamicEntity: m_entityManager.getEntities(DYNAMIC)) {
    ...
    for (auto &multiEntity: m_entityManager.getEntities()) {
      if (dynamicEntity->getComponent<CTransform>().prevPos == multiEntity->getComponent<CTransform>().prevPos)
        continue;// that is needed for not itherate on the same dynamic entity
          ...
          default:
            if (multiEntity->id() == TILE) {
              dynamicEntity->getComponent<CTransform>().pos.x -=
                  (dynamicEntity->getComponent<CTransform>().pos.x - dynamicEntity->getComponent<CTransform>().prevPos.x) /
                  (std::abs(dynamicEntity->getComponent<CTransform>().pos.x - dynamicEntity->getComponent<CTransform>().prevPos.x)) *
                  overlapRect.x;
